      ...
      onSubmit: async (values) => {
        ...
        try {
          const result = await fetch(
            `https://full-stack-ecommerce-website-server.vercel.app/user/registration`,
            {
              method: "post",
            ...
            if (values.role != "admin") {
              try {
                const cartResult = await fetch(
                  `https://full-stack-ecommerce-website-server.vercel.app/addCart`,
                  {
                    method: "post",
                    body: JSON.stringify({
                      userId: check.id,
      ...
      <form className={style.form} onSubmit={handleSubmit}>
        <div className={style.formDiv}>
          ...
          {errors.firstName && touched.firstName && (
            <div className={style.errorDiv}>*{errors.firstName}</div>
          )}
        </div>
        ...
        <div className={style.formDiv}>
          ...
          {errors.lastName && touched.lastName && (
            <div className={style.errorDiv}>*{errors.lastName}</div>
          )}
        </div>
        ...
        <div className={style.formDiv}>
          ...
          {errors.userName && touched.userName && (
            <div className={style.errorDiv}>*{errors.userName}</div>
          )}
        </div>
        ...
        <div className={style.formDiv}>
          <div>
            ...
            {errors.password && touched.password && (
              <div className={style.errorDiv}>*{errors.password}</div>
            )}
          </div>
        </div>
        ...
        <div className={style.formDiv}>
          ...
          {errors.confirmPassword && touched.confirmPassword && (
            <div className={style.errorDiv}>*{errors.confirmPassword}</div>
          )}
        </div>
        ...
        <div className={style.formDiv}>
          ...
          {errors.email && touched.email && (
            <div className={style.errorDiv}>*{errors.email}</div>
          )}
        </div>
        ...
        <div className={style.formDiv}>
          ...
          {errors.cnic && touched.cnic && (
            <div className={style.errorDiv}>*{errors.cnic}</div>
          )}
        </div>
...

        {
          <div className="w-[100%] mmd:w-[50%] text-sm mmd:text-lg flex flex-row items-center justify-between">
            <img
              className="w-12 h-12 mmd:w-20 mmd:h-20 rounded-[50%]  "
              src={avatar}
              alt=""
            />
            <input
              className=" text-xs mmd:text-lg"
              type="file"
              accept="image/"
